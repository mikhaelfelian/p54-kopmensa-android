...
import { Modal, View, Text, TouchableOpacity, TextInput, StyleSheet, FlatList, Alert, Platform, PermissionsAndroid, ScrollView } from "react-native";
import { FontAwesome, MaterialIcons } from "@expo/vector-icons";
import styles from "./styles";
import { useDispatch, useSelector } from "react-redux";
import { RootState } from "@/app/redux/store";
import { CheckVoucher, CreateTransaction, GetPaymentMethods } from "@/app/services/sales";
...
import { generateRandomString, toUrlEncoded } from "@/app/utils/converter";
...
const PaymentModal: React.FC<Props> = ({ visible, onClose, customerID, shiftCode }) => {
  ...
  const addMethod = () => {
    ...
    setMethods([...methods, { id: Date.now(), platform: defaultPlatform, amount: "0", note: "", voucherCode: "" }]);
  };
  ...
  const proceedTransaction = async () => {
    try {
      ...
      let isUsingVoucher = false;
      let voucherErrMsg = "";

      for (const item of methods) {
        if (item.platform === "Voucher") {
          isUsingVoucher = true;
          const _checkVoucherParam = {
            voucher_code: item.voucherCode,
          };
          const param = toUrlEncoded(_checkVoucherParam);

          try {
            const response = await CheckVoucher(param);
            isVoucherValid = response?.valid === true;
            if (!response?.valid) {
              voucherErrMsg = response?.message || "Voucher tidak valid.";
            }
          } catch (error: unknown) {
            if (error && typeof error === "object" && "response" in error) {
              const err = error as { response: any };
              voucherErrMsg = err.response?.data?.message || "Voucher tidak valid.";
            } else {
              voucherErrMsg = (error as Error)?.message || "Terjadi kesalahan saat memeriksa voucher.";
            }
          }
        }
      }

      if ((methods.filter((x) => Number(x.amount) > 0).length > 0 && !isUsingVoucher) || (isUsingVoucher && isVoucherValid)) {
        ...
        await CreateTransaction(param)
          ...
          .catch((error) => {
            ...
            Alert.alert("Error", error?.response?.data?.messages?.error || error?.message || "Please try again later.");
          });
      } else {
        Alert.alert("Error", voucherErrMsg?.length > 0 ? voucherErrMsg : "Silahkan lengkapi form pembayaran!");
      }
    } catch (error) {
        ...
        <View style={styles.container}>
          ...
          <ScrollView contentContainerStyle={{ flexGrow: 1 }} showsVerticalScrollIndicator={false}>
            ...
            <View style={{ marginTop: 12 }}>
              ...
              {methods.map((item) => (
                <View key={item.id.toString()} style={styles.paymentRow}>
                  <TouchableOpacity
                  ...
                  {item.platform === "Voucher" ? (
                    <TextInput
                      style={styles.amountInput}
                      keyboardType="default"
                      placeholder="Kode Voucher"
                      value={item.voucherCode}
                      onChangeText={(val) => setMethods((prev) => prev.map((m) => (m.id === item.id ? { ...m, voucherCode: val, amount: "0" } : m)))}
                    />
                  ) : (
                    <TextInput style={styles.amountInput} keyboardType="numeric" placeholder="0" value={item.amount} onChangeText={(val) => setMethods((prev) => prev.map((m) => (m.id === item.id ? { ...m, amount: val } : m)))} />
                  )}
